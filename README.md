...
复制 `.env.example` 文件�?`.env` 并根据需要修改配置：
...
- **API 地址**: http://localhost:7001/api
- **Swagger 文档**: http://localhost:7001/api-docs
- **健康检�?*: http://localhost:7001/api/health
...
npm run test:cov       # 生成测试覆盖率报�?
npm run test:e2e       # 运行端到端测�?
```

## API 文档

本项目集成了 Swagger，启动应用后访问 http://localhost:7001/api-docs 即可查看完整�?API 文档�?
...
1. **模块化设�?*：按功能划分模块，保持代码结构清�?
2. **DTO 验证**：所有输入都应通过 DTO 进行验证
3. **统一响应**：使�?`ApiSuccessResponse` 等装饰器
4. **错误处理**：使�?`BusinessException` 处理业务错误
5. **API 文档**：为所有接口添�?Swagger 装饰�?
6. **日志记录**：在关键操作处添加日�?
...
docker run -p 7001:7001 nestjs-api
```
